...
import { Send, ShieldCheck, Loader2, KeyRound, Sparkles } from 'lucide-react';

export const TelegramLoginModal: React.FC = () => {
  const { activeModal, closeModal, authTelegram, authYandex, openModal, showToast } = useGame();
  ...
  // 2. Listen for Yandex OAuth Popup Callback
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const targetBot = botUsername || 'MonopolyWebGameBot';
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && closeModal()}>
      <DialogContent className="max-w-sm text-center">
        ...
        <div className="flex flex-col gap-3 py-1">
          {/* Provider Tabs: Yandex ID vs Telegram */}
